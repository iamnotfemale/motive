...
## 10. 캔버스 조작 2차 (2026-09-19)

| 요청 | 구현 |
| --- | --- |
| 엔터 칠 때 화면 튐 | `focus({ preventScroll: true })`. 브라우저가 조상을 스크롤하던 게 원인 |
| 블록 드래그 중 화면 흔들림 | 카드 pointerdown 에서 기본 포커스를 막고 직접 포커스. 포인터 리스너는 ref 로 읽어 매 프레임 재등록하지 않음 |
| 선택 도구에서도 우클릭 이동 | 가운데·오른쪽 버튼은 어디서 눌러도 화면 이동 |
| 손 도구일 때 블록 클릭 | 변환 레이어를 `pointer-events-none` 으로 |
| Ctrl 확대가 마우스 기준 | 포인터 지점이 제자리에 남도록 pan 을 같이 이동 |
| 확대·축소 부드럽게 | 이동·확대를 `animateView` 하나로 모음 |
| 격자 옵션 | 도구 막대 → 왼쪽 레일 |
| 선 자동 정리 | 두 카드의 상대 위치로 마주보는 변을 고름. 같은 쌍의 선은 34px 씩 벌림 |
| 범위 선택 | 빈 곳 드래그. 자료 아이콘과 양 끝이 다 들어온 관계선까지 포함 |
| 여러 블록 → 새 그래프 | 선택 도구 막대의 `이 블록들로 새 그래프`. 관계·자료·상대 배치를 그대로 가져감 |
| 전체 화면 편집기 | Inspector `넓게 보기` → `components/WideEditor.tsx` |
| 카드에 첨부한 자료 목록 | 카드 위에 떨어뜨린 파일은 `Source.attachedTo` 로 묶여 카드 아래에 뜸 |
| 미확인 항목 클릭 | 해당 블록으로 이동 + 그 블록에 맞춰 확대 |
| 코드 블록 | python / c 만 |
| 하단 바(정의) | 가설 추가 · 생각 추가 두 개 |
| 탐색 단계 | 주소 읽기 제거 (자료함에 그대로 있음) |
| 인계 | `내가 확인할 항목` 제거 — 체크가 문서를 바꾸지 않는데 검증된 것처럼 읽혔다 |
| 전체 글자 크기 | 한 단계씩 상향, 본문 기본 14px |

## 11. 정리·선택·복제 (2026-09-19)

- **정리** = 관계 기준 격자 재배치. `lib/tidy.ts` 의 순수 함수로 빼고 `tidy.check.ts` 로 지킨다.
  근거와 검토 질문은 화살표 방향과 반대로, 자기가 가리키는 카드 **아래에** 달린다.
  붙어 있는 자료는 그 카드 바로 아래 같은 열에 놓인다.
  스펙 §12.2 때문에 사용자가 단추를 눌렀을 때만 돌고, ⌘Z 로 한 번에 취소된다.
- **범위 선택**에 자료 아이콘이 포함된다. 지우기·복제·접기도 묶음 전체에 걸린다.
- **문서로 보기 / 주변만 보기**는 카드 하나만 골랐을 때만 뜬다.
- **복제해 새 갈래 만들기** — 새 프로젝트를 만들지 않는다. 고른 블록을 같은 캔버스 오른쪽 빈 곳에
  복제하고, 고른 것들 사이의 관계도 함께 복제한다. 원본과 고르지 않은 카드는 그대로 남는다.

## 12. 단계 단추의 역할 분리 (2026-09-19)

같은 단어가 두 곳에서 다른 일을 하지 않도록 역할을 갈랐다.

- **아래 도구 막대 = 비추기.** 그 단계에서 볼 블록만 남기고 나머지를 흐린다 (집중 보기와 같은 방식).
  그래프를 바꾸지 않는다. 같은 단추를 다시 누르면 전체로 돌아온다.
- **왼쪽 레일 = 실행.** 파일을 붙이고, 근거를 찾고, 인계로 넘어간다.
- **만들기는 카드 액션과 `+` 에.** 도구 막대에서 `추가` 를 뺀 이유다.

| 단계 | 단추 | 비추는 것 |
| --- | --- | --- |
| 정의 | 가설 / 생각 | 가설 카드 / 메모 카드 |
| 탐색 | 자료 / 근거 찾기 | 올린 자료와 거기서 나온 근거 / 근거와 그 대상 |
| 검토 | 반박 / 미확인 | 반대 근거와 그 대상·어긋난 결정 / 미확인 항목으로 잡힌 카드 |
| 결정 | 해결안 / 결정 채택 | 해결안 카드 / 결정과 그 결정이 채택·기각한 것 |
| 인계 | 인계 / 내보내기 | (화면을 옮기므로 비추지 않음) |

`lib/spotlight.ts` 가 묶음을 만들고 `spotlight.check.ts` 가 지킨다 —
반박이 관계 없는 요구사항까지 비추거나, 미확인이 전부를 비추면 검사가 터진다.

카드 액션도 짧게 바꿨다: `문제 다듬기`→`다듬기`, `검토 질문 만들기`→`검토 질문`,
`결정 만들기`→`결정 채택`, `원문 보기`→`원문`, `수용 기준 편집`→`수용 기준`, `범위 변경`→`범위`.
검토 질문의 `상태 변경` 은 없앴다 — 상태는 카드 본문에서 바꾼다.
